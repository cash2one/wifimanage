...
class PubAction extends AdminAction{
	...
	public  function getpubadrpt(){
    	...
    	switch(strtolower($way)){
    		case "history":
    			$sql=" select t,CONCAT(CURDATE(),' ',t,'点') as showdate, COALESCE(showup,0)  as showup,COALESCE(count,0)  as count, COALESCE(hit,0)  as hit,COALESCE(hit/showup*100,0) as rt from ".C('DB_PREFIX')."hours a left JOIN ";
                $sql.="(select thour, sum(showup)as showup,count(apid) as count,sum(hit) as hit from ";
                $sql.="(select  add_hour as thour,showup,hit,apid from ".C('DB_PREFIX')."adcount";
